---
name: finn-illustrations
description: 生成 Finn 风格的中文正文配图。用于用户要求为中文文章、帖子、博客、Notion 文档、工作流文档、方法论、流程、结构、状态、隐喻或观点生成“手绘”“正文配图”“文章插图”“配图建议”“shot list”“去标题/改图”等任务；默认使用 Finn IP、纯白手绘、少量蓝黄红批注、简洁清爽但有个人识别度的视觉风格。
---
...
默认视觉 IP 是 Finn：戴细边圆框眼镜、蓝黑短发、白衬衫和蓝色针织衫的年轻男生。Finn 必须参与画面的核心动作，不能只是站在旁边当头像装饰。
...
- `references/finn-ip.md`：Finn IP 的形象、性格、动作库和禁忌。
...
- `assets/finn-avatar.jpg`：默认头像参考，只用于校准人物，不要求每张图都画头像圆框。
...
如果用户明确要求“生成 / 输出 / 做图 / 帮我生成”，不要停下来等确认；用可用图像模型每张单独生成。不要把多张图拼在一张里。
